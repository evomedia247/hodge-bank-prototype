...
$(document).ready(function () {
	...
	var appViewModel = function () {
		...
		self.inPayment = ko.observable();
		self.inPayment2 = ko.observable();
		self.showHidden = ko.observable();
		...
		self.checkIfShowApp2 = function() {
			if (vm.DataSwitcher().includes(13)) {
				return this.shouldShow(true);
			} else if (vm.DataSwitcher().includes(!13)) {
				return this.shouldShow(false);
			}
		};
		...
		previousNavigation = function() {
			if (vm.DataSwitcher().includes(7)) {
				...
				window.location.href = "/incomeQuestion2-sidenav.html";
			}
		}
